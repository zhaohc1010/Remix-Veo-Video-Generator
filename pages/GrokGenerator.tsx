...
import { generateGrokVideo } from '../services/apiService';
import { GrokGenerationConfig, LogEntry, GenerationStatus, HistoryItem } from '../types';
...
const GrokGenerator: React.FC = () => {
  ...
  const [history, setHistory] = useState<HistoryItem[]>(() => {
    try {
      const saved = localStorage.getItem('grok_history');
  ...
  useEffect(() => {
    ...
    processingTasks.forEach(task => {
      // Re-run the polling for this task ID
      // We don't have the original config, but we only need the task ID and API key to poll
      ...
      // Import pollGrokTaskStatus from apiService (need to export it first)
      import('../services/apiService').then(({ pollGrokTaskStatus }) => {
        pollGrokTaskStatus(task.id, apiKey, (log) => {
          ...
          else if (responseData.data?.video_url) videoUrl = responseData.data.video_url;
  ...
  useEffect(() => {
    localStorage.setItem('grok_history', JSON.stringify(history));
  }, [history]);
  ...
  const runGenerationTask = async (taskId: string, config: GrokGenerationConfig) => {
    const logPrefix = `[Task-${taskId.slice(-4)}]`;

    try {
      const responseData = await generateGrokVideo(config, (log) => {
        addLog({ ...log, message: `${logPrefix} ${log.message}` });
      });
  ...
  const handleGenerate = () => {
    ...
    const config: GrokGenerationConfig = {
      apiKey,
      prompt,
      model,
      ...
      <header className="bg-dark-800 border-b border-gray-800 sticky top-0 z-40">
        <div className="max-w-7xl mx-auto px-4 h-16 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-lg font-bold tracking-tight">Grok 视频生成</h1>
          ...
          <div className="bg-dark-800 border border-gray-700 rounded-xl p-6 shadow-xl">
            ...
            <div className="space-y-5">
              ...
              <button
                ...
                className={`w-full py-4 rounded-xl font-bold text-lg flex items-center justify-center gap-2 shadow-lg transition transform active:scale-[0.98] bg-gradient-to-r from-brand-600 to-cyan-500 hover:from-brand-500 hover:to-cyan-400 text-white shadow-brand-500/25`}
              >
                ...
                {currentVideoUrl && (
                    <div className="w-full h-full flex flex-col items-center">
                        ...
                        <div className="mt-4 flex gap-3">
                          <button 
                            onClick={() => downloadVideo(currentVideoUrl!, `grok-video-${Date.now()}.mp4`)}
                 ...
                 <div key={item.id} className="bg-dark-900 border border-gray-800 rounded-lg p-3 flex gap-4 hover:border-brand-500/30 transition group">
                    ...
                    {item.videoUrl && (
                       <div className="flex flex-col justify-center gap-2">
                          ...
                          <button 
                             onClick={() => downloadVideo(item.videoUrl!, `grok-${item.id}.mp4`)}
